...
module Decoder(
    ...
    output reg ALUSrcB,		// Asserted by all instructions which use an immediate (load, store, lui, auipc, DPImm). Needs to be expanded to a 2-bit signal to support link functionality for jal, jalr. Change wire width in RV.v as well
    ...
    output reg MCycleStart,
    output reg [1:0] MCycleOp,
    output reg MCycleSelect
    ); 
