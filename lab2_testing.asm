...
main:
    ...
    mul s2, s0, s1     # s2 = s0 * s1 (s2 should be -15)

    # Test unsigned multiplication (high bits)
    li s0, 0x80000000  # Load a large number into s0
    li s1, 2           # Load 2 into s1
    mulhu s3, s0, s1   # s3 = high 32 bits of unsigned(s0) * unsigned(s1)

    # Test divu (unsigned division)
    li s0, 20          # Load 20 into s0
    li s1, 3           # Load 3 into s1
    divu s4, s0, s1    # s4 = s0 / s1 (s4 should be 6)
